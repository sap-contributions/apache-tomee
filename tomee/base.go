...
func NewBase(
	...
	b := Base{
		...
		LayerContributor: libpak.NewLayerContributor("Apache Tomee Support", map[string]interface{}{
...
func (b Base) Contribute(layer libcnb.Layer) (libcnb.Layer, error) {
	...
	return b.LayerContributor.Contribute(layer, func() (libcnb.Layer, error) {
		...
		environmentPropertySourceDisabled := b.ConfigurationResolver.ResolveBool("BP_TOMEE_ENVIRONMENT_PROPERTY_SOURCE_DISABLED")
...
func (b Base) ContributeExternalConfiguration(layer libcnb.Layer) error {
	...
	c := 0
	if s, ok := b.ConfigurationResolver.Resolve("BP_TOMEE_EXT_CONF_STRIP"); ok {
