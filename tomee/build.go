...
func (b Build) Build(context libcnb.BuildContext) (libcnb.BuildResult, error) {
	...
	h, be := libpak.NewHelperLayer(context.Buildpack, "access-logging-support", "environment-property-source-support")
	h.Logger = b.Logger
	...
	if uri, ok := cr.Resolve("BP_TOMEE_EXT_CONF_URI"); ok {
		...
		s, _ := cr.Resolve("BP_TOMEE_EXT_CONF_SHA256")
	...
	result.Processes = append(result.Processes,
		...
		libcnb.Process{Type: "tomee", Command: command, Arguments: arguments, Direct: true},
...
func (b Build) ContextPath(configurationResolver libpak.ConfigurationResolver) string {
	cp := "ROOT"
	if s, ok := configurationResolver.Resolve("BP_TOMEE_CONTEXT_PATH"); ok {
		cp = s
	}
...
func (b Build) tinyStartCommand(homePath, basePath string, loggingDep libpak.BuildpackDependency) (string, []string) {
	...
	b.Logger.Header(color.YellowString("WARNING: Tomee will run on the Tiny stack which has no shell. Due to this, some configuration options such as `bin/setenv.sh` and setting `CATALINA_*` environment variables, will not be available"))
